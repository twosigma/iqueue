/*      $TwoSigma: iqueue-main.c,v 1.17 2012/02/02 20:48:52 thudson Exp $       */
...
#include "tsutil.h"
#include "tsio.h"
#include "tsutil_dcat.h"
#include "iqueue.h"
#include "iqueue_cpio.h"
#include "tsclock.h"
#include "segfault.h"
...
)
{
    ...
    TSLOGX(TSERROR, "message too long! limit is %zu bytes", len);
    return -1;
}
...
)
{
    ...
    if (rc != 0)
    {
	TSLOGX(TSERROR, "%s: Update failed rc=%d", iqueue_name(iq), rc);
	return EXIT_FAILURE;
    }
...
static int
iqueue_copyin_from_stdin(
    const char * const filename,
    const bool do_follow
)
{
  return iqueue_create_and_copyin_from_file(filename, do_follow, stdin);
}
...
retry:
	...
	if (iqueue_reopen_wait(iq) < 0)
	{
	    TSLOG(TSERROR, "%s: Unable to reopen", iqueue_name(iq));
	    return EXIT_FAILURE;
	}
...
)
{
    ...
    TSLOGX(TSINFO, "%s: iqueue has been sealed", iqueue_name(iq));
...
)
{
    ...
    if (rc != 0)
    {
	TSLOGX(TSERROR, "%s: Unable to seal: %s",
	    old_name,
    ...
    TSLOGX(TSINFO, "%s: archived", iqueue_name(iq));
...
)
{
	...
	TSLOGX(TSINFO, "%s: Writer %u.0x%"PRIx64" value %"PRIu64,
	    iqueue_name(iq),
	    table_id,
	    id,
	    value
    ...
    TSLOGX(TSWARN, "%s: Writer %u.0x%"PRIx64" tried to write %"PRIu64", current value %"PRIu64,
	iqueue_name(iq),
	table_id,
	id,
	value,
...
)
{
	...
	int c = getopt_long(
	    argc,
	    argv,
	    "h?f:CGaFz0bAHI:D:n:B:E:swNl12W:",
	    long_options,
	    &option_index
	);
	...
	case '1': copyin_flag = 1; writable = true; break;
	case '2': copyout_flag = 1; break;
    ...
    if (copyin_flag)
	return iqueue_copyin_from_stdin(iqueue_file, follow);
	...
	if (iqueue_creation(iq) != creation)
	    TSLOGX(TSINFO, "%s: iqueue already existed", iqueue_file);
	return EXIT_SUCCESS;
    }
    ...
    if (copyout_flag)
	return iqueue_copyout(iq, follow, begin_entry, end_entry);
    if (append)
	...
	if (binary_out)
	{
	    ssize_t wlen = tsio_write_all(STDOUT_FILENO, data, len);
	    if ((size_t) wlen != len)
